<?php
    ...
    if (isset($_SESSION['id'])) {
        ...
        if(array_key_exists("name", $_GET)) {
            if(!empty($_GET['name'])) {
                $filter['search'] = [['d.name'], $_GET['name']];
            }
        }

        $dish_data = get_all_dishes($filter, ['offset'=> $offset, 'total_records_per_page' => TOTAL_RECORDS_PER_PAGE]);
                        ...
                        <form method="get">
                            <div id="form-search">
                                <div id="search-field" class="input-control">
                                    <input type="text" name="name" class="input-field input-xs" value="<?= (isset($_GET['name'])) ? $_GET['name'] : ''; ?>" placeholder="Search a dish" />
                                </div>
                                <div id="search-btn" class="input-control">
                                    <input type="submit" name="submit" class="btn btn-sm btn-rounded" value="Search" />
                                </div>
                            </div>
                        </form>            
                    </div>
